...
async function topBrands(limit) {
  const rows = db.prepare(`SELECT brand, COUNT(*) c FROM products WHERE brand IS NOT NULL AND brand <> '' AND LOWER(brand) NOT IN (SELECT DISTINCT LOWER(advertiser) FROM products WHERE advertiser IS NOT NULL AND advertiser <> '') GROUP BY brand ORDER BY c DESC LIMIT ?`).all(limit || 12);
