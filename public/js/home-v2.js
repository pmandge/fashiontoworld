  ...
  (function () {
    ...
    function draw(cat) {
      rail.innerHTML = STORES.filter(function (s) { return cat === 'all' || (s.cats || []).indexOf(cat) > -1; }).map(function (s) {
        var logo = s.d ? ('<img alt="' + esc(s.name) + '" src="https://www.google.com/s2/favicons?domain=' + s.d + '&sz=128" onerror="this.replaceWith(document.createTextNode(\'' + s.name.trim()[0] + '\'))">') : s.name.trim()[0];
